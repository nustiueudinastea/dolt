...
func NewParquetRowWriterForFile(ctx context.Context, outSch schema.Schema, destName string) (*ParquetRowWriter, error) {
	primaryKeySchema, err := sqlutil.FromDoltSchema(ctx, "", "", outSch)
	if err != nil {
		return nil, err
	}
