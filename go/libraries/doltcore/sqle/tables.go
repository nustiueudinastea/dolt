...
import (
	...
	"github.com/nustiueudinastea/dolt/go/libraries/doltcore/sqle/fk"
	...
	"github.com/nustiueudinastea/dolt/go/libraries/doltcore/table/editor/creation"
...
func (t *DoltTable) GetDeclaredForeignKeys(ctx *sql.Context) ([]sql.ForeignKeyConstraint, error) {
	...
	for i, fk := range declaredFks {
		...
		toReturn[i], err = sqlutil.ToForeignKeyConstraint(fk, t.db.Name(), t.sch, parentSch)
		if err != nil {
			return nil, err
		}
	}
...
func (t *DoltTable) GetReferencedForeignKeys(ctx *sql.Context) ([]sql.ForeignKeyConstraint, error) {
	...
	return fk.GetReferencedForeignKeys(ctx, root, t.db.Name(), t.TableName(), t.sch)
}
...
func (t *AlterableDoltTable) getFirstAutoIncrementValue(
	...
	for {
		...
		cmp, err := columnType.Compare(ctx, initialValue, r[colIdx])
		if err != nil {
			return 0, err
		}
		if cmp < 0 {
