...
func (cmd ImportCmd) Exec(ctx context.Context, commandStr string, args []string, _ *env.DoltEnv, cliCtx cli.CliContext) int {
	ap := cmd.ArgParser()
	...
	if err != nil {
		errorText := err.Error()
		switch {
		case strings.Contains(errorText, "nothing to commit"):
