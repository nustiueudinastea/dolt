...
func TestArchiveChunkGroup(t *testing.T) {
	...
	assertIntBetween(t, cg.totalBytesSavedWDict, 8650, 8720)
...
func TestArchiveConjoinAll(t *testing.T) {
	...
	err := awCombined.conjoinAll(context.Background(), readersToSource(readers), NewUnlimitedMemQuotaProvider(), &Stats{})
	assert.NoError(t, err)
...
func TestArchiveConjoinAllComprehensive(t *testing.T) {
	...
	err := aw1.conjoinAll(context.Background(), readersToSource(readers), NewUnlimitedMemQuotaProvider(), &Stats{})
	assert.NoError(t, err)
	...
	err = aw2.conjoinAll(context.Background(), readersToSource(allReadersForSecondConjoin), NewUnlimitedMemQuotaProvider(), &Stats{})
	assert.NoError(t, err)
